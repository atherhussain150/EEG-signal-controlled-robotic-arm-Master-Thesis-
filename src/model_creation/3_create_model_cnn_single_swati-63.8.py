...
orig_epochs = loadtxt('my_1536_window_RT_falvor_specialization_0_1000.csv', delimiter=',')
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=75, kernel_size=6, kernel_regularizer=L2(L2_Regularization), bias_regularizer=L2(L2_Regularization), activity_regularizer = L2(L2_Regularization), kernel_constraint=min_max_norm(min_value=negative_wt, max_value=positive_wt), padding='valid', activation='relu', strides=2, input_shape=(500, 64)))
...
model.add(Conv1D(filters=30, kernel_size=3, kernel_regularizer=L2(L2_Regularization), bias_regularizer=L2(L2_Regularization), activity_regularizer = L2(L2_Regularization), kernel_constraint=min_max_norm(min_value=negative_wt, max_value=positive_wt), padding='valid', activation='relu', strides=1))
...
# Compile the model  
adam = Adam(learning_rate=0.00028)
...
hist = model.fit(input, Y_train, batch_size=40, epochs=85, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None, callbacks=[es, mc])
