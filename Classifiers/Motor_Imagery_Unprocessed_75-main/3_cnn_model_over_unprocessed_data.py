...
# load the train data
X = loadtxt('/content/input_unprocessed.csv', delimiter=',')
print(X.shape)
...
input = rScaler.fit_transform(train_data[:, 0:131072])
testinput = rScaler.fit_transform(X_test_tmp[:,0:131072])
...
testinput = testinput.reshape(len(testinput), 64, 2048)
testinput = testinput.transpose(0, 2, 1)
print (testinput.shape)

# Create the model
model=Sequential()
model.add(Conv1D(filters=10, kernel_size=6, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), data_format='channels_last', padding='valid', activation='relu', strides=3, input_shape=(2048, 64)))
model.add(AveragePooling1D(pool_size=3))
model.add(Conv1D(filters=30, kernel_size=6, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), data_format='channels_last', padding='valid', activation='relu', strides=3))
model.add(Dropout(0.5))
model.add(GlobalAveragePooling1D())
model.add(Dense(10, activation='relu', kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2)))
model.add(Dense(2, activation='softmax'))

model.summary()

# Compile the model   
adam = Adam(learning_rate=0.00012)
...
hist = model.fit(input, Y_train, batch_size=32, epochs=120, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None, callbacks=[es, mc])
